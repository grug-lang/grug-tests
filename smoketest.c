...
static void on_fn_dispatcher(const char *on_fn_name, const char *grug_file_path, const union grug_value args[]) {
    ...
    if (starts_with(grug_file_path, "tests/err_runtime/all/")) {
        ...
        CALL(initialize_bool,
            grug_bool(
                CALL(has_resource, grug_string("tests/ok/entity_and_resource_as_subexpression/foo.txt"))._bool
        ...
        CALL(draw, grug_string("tests/ok/resource_can_contain_dot_1/.foo"));
    } else if (starts_with(grug_file_path, "tests/ok/resource_can_contain_dot_2/")) {
        CALL(draw, grug_string("tests/ok/resource_can_contain_dot_2/foo."));
    } else if (starts_with(grug_file_path, "tests/ok/resource_can_contain_dot_3/")) {
        CALL(draw, grug_string("tests/ok/resource_can_contain_dot_3/foo.bar"));
    } else if (starts_with(grug_file_path, "tests/ok/resource_can_contain_dot_dot_1/")) {
        CALL(draw, grug_string("tests/ok/resource_can_contain_dot_dot_1/..foo"));
    } else if (starts_with(grug_file_path, "tests/ok/resource_can_contain_dot_dot_2/")) {
        CALL(draw, grug_string("tests/ok/resource_can_contain_dot_dot_2/foo.."));
    } else if (starts_with(grug_file_path, "tests/ok/resource_can_contain_dot_dot_3/")) {
        CALL(draw, grug_string("tests/ok/resource_can_contain_dot_dot_3/foo..bar"));
    } else if (starts_with(grug_file_path, "tests/ok/resource_duplicate/")) {
        CALL(draw, grug_string("tests/ok/resource_duplicate/foo.txt"));
        CALL(draw, grug_string("tests/ok/resource_duplicate/bar.txt"));
        CALL(draw, grug_string("tests/ok/resource_duplicate/bar.txt"));
        CALL(draw, grug_string("tests/ok/resource_duplicate/baz.txt"));
