...
static size_t game_fn_eval_order_1_call_count;
...
static size_t game_fn_vec_number_with_capacity_call_count;
...
union grug_value game_fn_d_magic(struct grug_state* grug_state, const union grug_value args[], const struct grug_type generics[]) {
	(void)generics;
	return game_fn_magic(grug_state, args, generics);
}
...
static bool game_fn_eval_order_1_called_before_eval_order_2;
union grug_value game_fn_eval_order_1(struct grug_state* grug_state, const union grug_value args[], const struct grug_type generics[]) {
	(void)generics;
	(void)grug_state;
	(void)args;
	ASSERT_16_BYTE_STACK_ALIGNED();
	game_fn_eval_order_1_call_count++;

	game_fn_eval_order_1_called_before_eval_order_2 = game_fn_eval_order_2_call_count == 0;

	return grug_number(1.0);
}
union grug_value game_fn_eval_order_2(struct grug_state* grug_state, const union grug_value args[], const struct grug_type generics[]) {
	(void)generics;
	(void)grug_state;
	(void)args;
	ASSERT_16_BYTE_STACK_ALIGNED();
	game_fn_eval_order_2_call_count++;

	return grug_number(2.0);
}
...
union grug_value game_fn_Utils_fail(struct grug_state* grug_state, const union grug_value args[], const struct grug_type generics[]) {
	return game_fn_cause_game_fn_error(grug_state, args, generics);
}
...
size_t vec_number_last_capacity;
union grug_value game_fn_vec_number_with_capacity(struct grug_state* grug_state, const union grug_value args[], const struct grug_type generics[]) {
	(void)generics;
	(void)grug_state;
	ASSERT_16_BYTE_STACK_ALIGNED();
	game_fn_vec_number_with_capacity_call_count++;
	vec_number_last_capacity = (size_t)args[0]._number;

	return game_fn_vec_number_new(grug_state, args, generics);
}
...
union grug_value game_fn_default(struct grug_state* grug_state, const union grug_value args[], const struct grug_type generics[]) {
    switch (generics[0].type) {
        case GRUG_TYPE_ENUM_STRING:
            return game_fn_default_string(grug_state, args, generics);
        case GRUG_TYPE_ENUM_ID:
            if (streq(generics[0].data.id.name, "Vec")) {
                return game_fn_vec_number_new(grug_state, args, generics);
            }
    ...
    fprintf(stderr, "Error: Unsupported default instantiation\n");
    exit(EXIT_FAILURE);
}
...
static void compare_nodes(cJSON *exp, cJSON *act, const char *path) {
	...
	if (cJSON_IsString(exp)) {
		if (!streq(exp->valuestring, act->valuestring)) {
...
static void reset(void) {
	...
	game_fn_eval_order_1_call_count                   = 0;
...
static void ok_evaluation_order_is_left_to_right_1(struct grug_state* grug_state, struct grug_entity_id* entity) {
	assert_call_count(eval_order_1, 0);
	assert_call_count(eval_order_2, 0);
	assert_call_count(initialize, 0);
	call_export_fn_argless(grug_state, entity, "a");
	assert_call_count(eval_order_1, 1);
	assert_call_count(eval_order_2, 1);
	assert_call_count(initialize, 1);

	assert_number(game_fn_initialize_x, 3.0);

	assert_true(game_fn_eval_order_1_called_before_eval_order_2);
}
...
static void ok_evaluation_order_is_left_to_right_2(struct grug_state* grug_state, struct grug_entity_id* entity) {
	...
	assert_number(game_fn_max_x, 1.0);
	assert_number(game_fn_max_y, 2.0);

	assert_true(game_fn_eval_order_1_called_before_eval_order_2);
}

static void ok_evaluation_order_is_left_to_right_3(struct grug_state* grug_state, struct grug_entity_id* entity) {
	assert_call_count(initialize, 0);
	call_export_fn_argless(grug_state, entity, "a");
	assert_call_count(initialize, 1);
...
static void ok_evaluation_order_is_left_to_right_4(struct grug_state* grug_state, struct grug_entity_id* entity) {
	assert_call_count(max, 0);
	call_export_fn_argless(grug_state, entity, "a");
	assert_call_count(max, 1);
...
static void ok_f32_passed_to_on_fn(struct grug_state* grug_state, struct grug_entity_id* entity) {
	assert_call_count(sin, 0);
	call_export_fn(grug_state, entity, "a", (const union grug_value[]){{._number=42.0}}, 1);
	assert_call_count(sin, 1);
...
static void ok_on_fn_overwriting_param(struct grug_state* grug_state, struct grug_entity_id* entity) {
	assert_call_count(initialize, 0);
	assert_call_count(sin, 0);
	call_export_fn(grug_state, entity, "a", (const union grug_value[]){{._number=2.0}, {._number=3.0}}, 2);
	assert_call_count(initialize, 1);
	assert_call_count(sin, 1);
...
static void ok_resource_can_contain_dot_2(struct grug_state* grug_state, struct grug_entity_id* entity) {
	...
	assert_string(game_fn_draw_sprite_path, "ok/resource_can_contain_dot_2/foo.bar");
}
...
static void ok_resource_can_contain_dot_dot_dot(struct grug_state* grug_state, struct grug_entity_id* entity) {
	...
	assert_string(game_fn_draw_sprite_path, "ok/resource_can_contain_dot_dot_dot/...foo");
}
...
static void add_error_tests(void) {
	...
	ADD_TEST_ERROR_FILE_NAME(file_name_missing_period, "input-Dgrug");
...
static void add_ok_tests(void) {
	...
	ADD_TEST_OK(evaluation_order_is_left_to_right_3, "D");
	ADD_TEST_OK(evaluation_order_is_left_to_right_4, "D");
...
static void add_runtime_error_tests(void) {
	...
	ADD_TEST_RUNTIME_ERROR(game_fn_error_in_static_method, "D");
